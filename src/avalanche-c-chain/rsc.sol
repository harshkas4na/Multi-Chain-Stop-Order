...
contract PangolinDemoStopOrderReactive is IReactive, AbstractReactive {
    ...
    uint256 private constant AVALANCHE_CHAIN_ID = 43114;  // Changed from Sepolia to Avalanche
    // Pangolin's Sync event has the same signature/topic as Uniswap V2's
    uint256 private constant PANGOLIN_SYNC_TOPIC_0 = 0x1c411e9a96e071241c2f21f7726b17ae89e3cab4c78be50e062b03a9fffbbad1;
    ...
    constructor(
        ...
        if (!vm) {
            service.subscribe(
                AVALANCHE_CHAIN_ID,  // Changed from Sepolia to Avalanche
                pair,
                PANGOLIN_SYNC_TOPIC_0,  // Renamed but value is same
                REACTIVE_IGNORE,
                REACTIVE_IGNORE,
                REACTIVE_IGNORE
            );
